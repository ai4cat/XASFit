...
file1 = "/mnt/data/SYC/xasfitting/exp/CoCe_Co_K_merged.txt"
root_dir = "/mnt/data/SYC/xasfitting/xmudat/1.CoCe"  
prominence = 0.03
distance = 5
fit_half_window = 10
...
def detect_micro_fluctuation_single_peak(x, y, start, end, window_size=5, range_thresh=0.001):
...
# reference
x1, y1 = load_xy(file1)
lim1 = (x1 >= 7660) & (x1 <= 7840)
x1, y1 = x1[lim1], y1[lim1]
mus1, prom1 = detect_peaks_with_refine(x1, y1)
mus1[1]=mus1[1]-1.5
ref = np.array([7710, 7735, 7740, 7775])
mus1 = np.concatenate([ref, mus1])

results = []

for dirpath, dirnames, filenames in os.walk(root_dir):
    if dirpath.endswith("K"):  # K
        file2 = os.path.join(dirpath, "xmu_extracted.dat")
        if os.path.exists(file2):
            try:
                x2, y2 = load_xy(file2)
                x2 = x2 - 3.5
                lim2 = (x2 >= 7660) & (x2 <= 7840)
                xl, yl = x2[lim2], y2[lim2]
                #lim2 = (x2 >= 5700) & (x2 <= 5770)
                x2, y2 = x2[lim2], y2[lim2]
                mus2, prom2 = detect_peaks_with_refine(x2, y2)

                micro_peaks = detect_micro_fluctuation_single_peak(x2, y2, 7705, 7720)
                micro_peaks2 = detect_micro_fluctuation_single_peak(x2, y2, 7730, 7739)
                micro_peaks3 = detect_micro_fluctuation_single_peak(x2, y2, 7740, 7749)
                micro_peaks4 = detect_micro_fluctuation_single_peak(x2, y2, 7770, 7780)
                mus2 = np.concatenate([mus2, micro_peaks, micro_peaks2, micro_peaks3, micro_peaks4])
                mus2 = np.sort(mus2)
...
out_csv = "CoCe_K_peak_fitting_results.csv"
